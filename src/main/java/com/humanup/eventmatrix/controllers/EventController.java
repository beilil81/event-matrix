...
public class EventController {
    ...
    @Operation(
            summary = "Create Event", description = " Create new Event by Libelle, Description ...", tags = {"event"})
    @RequestMapping(
            value = "/event",
    ...
    @Operation(summary = "Find all events by person", description = "Find all event by person emailAdress", tags = {"event"})
    @RequestMapping(value = "/event/all/person", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity getEventPerson(@RequestParam(value = "emailAddress", defaultValue = "mohamedbeilil83@gmail.com") String emailAddress) {
        Optional<List<EventVO>> findEvent = Optional.ofNullable(eventBS.findEventByEmailPerson(emailAddress));
